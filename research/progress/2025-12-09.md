...
- El binario `soundtable-tracker` se ha estructurado para soportar dos modos de ejecución diferenciados:
  - Modo `--mode=synthetic` (por defecto), que conserva el comportamiento anterior: envía un único objeto sintético `/soundtable/object` con `trackingId=1` y `logicalId="osc1"` moviéndose horizontalmente.
  - Modo `--mode=live`, que inicializa un nuevo `soundtable::tracker::TrackerEngine` y pasa cada frame capturado para que devuelva una lista de `TrackedObject` con coordenadas normalizadas y ángulo en radianes.
...
- `tracker/src/main.cpp` ahora:
  ...
  - En modo live, inicializa `TrackerEngine` con la resolución actual de la cámara y, en cada frame, obtiene una lista de objetos y envía un mensaje `/soundtable/object` por cada uno, usando una función `mapLogicalId(int markerId)` para traducir ids numéricos a `logicalId` (por ahora mapeo fijo `1→osc1`, `2→filter1`).
  - Mantiene el cálculo de FPS y los mensajes de log con prefijo `[soundtable-tracker]`.
- `tracker/CMakeLists.txt` se ha actualizado para compilar e incluir los nuevos archivos de tracking en el ejecutable `soundtable-tracker`.
...
- UI JUCE mínima (`soundtable-core`):
...
- Binario `soundtable-tracker` basado en OpenCV (`tracker/src/main.cpp`):
  - Abre la cámara por defecto.
  - Bucle principal que lee frames y calcula FPS aproximados cada segundo.
  - Mensajes de log en inglés con prefijo `[soundtable-tracker]`.
  - Emisor OSC mínimo (`OscSender`) que envía mensajes `/soundtable/object` sintéticos
...
- Nueva clase `TrackingOscReceiver` en `core/src/TrackingOscReceiver.{h,cpp}`:
  ...
  - Protocolo OSC minimalista para MVP:
    - `/soundtable/object` (int32 trackingId, string logicalId, float x, float y, float angleRadians) → `Scene::UpsertObject`.
    - `/soundtable/remove` (int32 trackingId) → `Scene::RemoveObject`.
...
- Añadido `AudioEngine` en `core/src/AudioEngine.{h,cpp}`:
  ...
  - Punto de partida para evolucionar hacia un grafo de módulos que mapee `soundtable::ModuleType`/`AudioModule` a nodos de procesamiento.
...
- **Tracking/protocolo**:
  - Extender `soundtable-tracker` para enviar mensajes OSC reales compatibles con el protocolo `/soundtable/object` y `/soundtable/remove` (más allá de los datos sintéticos actuales).
...
- **UI/Escenas**:
  - Completar la deserialización (carga) de escenas usando el formato line-based `soundtable_scene_v1` de `SceneSerialization` o migrar a un formato JSON cuando se introduzca una librería dedicada.
...
- Se ha introducido un estado de mute por conexión (`mutedConnections_` en `MainComponent`) indexado por un identificador estable de `soundtable::Connection`.
...
- Estos módulos todavía no se usan en la UI ni en los tests, pero proporcionan una capa de modelo coherente para implementar a continuación el loader de archivos `.rtp` y poblar `soundtable::Scene` con instancias que reflejen fielmente los patches originales de Reactable.

### Loader de patches Reactable `.rtp` y hardlinks
- Añadido `core/src/core/ReactableRtpLoader.{h,cpp}` con una API `LoadReactablePatchFromString` / `LoadReactablePatchFromFile` que parsea directamente archivos `.rtp` XML (sin cambiar su estructura) y construye un `soundtable::Scene` con módulos concretos (`OutputModule`, `DelayModule`, `OscillatorModule`, `SequencerModule`, `LoopModule`, etc.) y `ObjectInstance` para cada `<tangible>`.
...
- Implementada la traducción de `<hardlink to="..." />` dentro de tangibles `Delay` a conexiones reales del modelo: para cada hardlink, se crea un `soundtable::Connection` que une el puerto `out` del módulo delay con el puerto `in` del módulo destino (por ejemplo, `Output` con id `-1`). Estas conexiones se marcan ahora explícitamente en el modelo con `is_hardlink = true`.
...
- Se añaden helpers internos para:
  - Detectar si una `soundtable::Connection` es de audio (consultando los `PortDescriptor` de `AudioModule` y, en su defecto, las flags `produces_audio`/`consumes_audio`).
...
- La aplicación JUCE principal (`soundtable-core`) ahora intenta cargar automáticamente el patch Reactable por defecto ubicado en `com.reactable/Resources/default.rtp` al inicializar `MainComponent`.
- Se reutiliza el loader existente `LoadReactablePatchFromFile` de `core/src/core/ReactableRtpLoader.{h,cpp}` para poblar directamente un `soundtable::Scene` con módulos, conexiones y objetos a partir del `.rtp` original.
...
- `soundtable::ObjectInstance` se ha ampliado con un flag booleano `docked`, expuesto mediante `bool docked() const`, que indica si el tangible procede de un elemento `docked="1"` en el `.rtp`.
...
- Se ha alineado el uso de los helpers de UI definidos en `MainComponentHelpers.h` (`makeConnectionKey`, `makeObjectPairKey`, `makeModulePairKey`, `isConnectionGeometricallyActive`) para que se invoquen siempre cualificados como `soundtable::ui::...` desde `MainComponent.cpp`.
- Esto corrige errores de compilación recientes por símbolos no resueltos y deja más clara la separación entre la lógica de modelo (`soundtable`) y las utilidades puramente gráficas/de UI (`soundtable::ui`).

### Helper genérico `loadFile` para recursos de Reactable
- Se ha extraído la lógica de búsqueda de recursos (antes duplicada en `MainComponent` y `MainComponent_Atlas`) a un helper `soundtable::ui::loadFile` definido en `MainComponentHelpers.{h,cpp}`.
...
- El protocolo OSC de tracking (`TrackingOscReceiver`) interpreta ahora el quinto argumento de `/soundtable/object` como ángulo en grados dentro del rango `[0, 360]` y lo convierte internamente a radianes antes de poblar `soundtable::ObjectInstance`, manteniendo la semántica del modelo (`angle_radians`) coherente con el loader `.rtp`.
...
- Los objetos que desaparecen de la escena (se borran vía `/soundtable/remove` o por otros caminos) se eliminan también del mapa `lastObjectAngleDegrees_` en cada frame, evitando fugas de memoria y dejando el tracking preparado para nuevas apariciones con un historial limpio.

### Alineación del ángulo OSC entre tracker y core
- `tracker/src/OscSender.{h,cpp}` espera ahora explícitamente el ángulo en **grados** en `sendObject` y lo empaqueta tal cual en el mensaje `/soundtable/object` (tipo `float`).
...
- `soundtable::Scene::UpsertObject` garantiza ahora que solo exista un `ObjectInstance` por cada `logical_id` en la escena: antes de insertar/actualizar el nuevo objeto, recorre el mapa interno `objects_` y elimina cualquier entrada previa cuyo `logical_id()` coincida pero cuyo `tracking_id()` sea distinto.
...
- El manejador `/soundtable/remove` en `TrackingOscReceiver` deja de llamar directamente a `Scene::RemoveObject` y, en su lugar, busca el `ObjectInstance` asociado al `trackingId` que llega por OSC.
...
- El ejecutable `soundtable-tracker` ahora enlaza directamente con el código original de reacTIVision (`reacTIVision/ext/libfidtrack`), añadiendo a su target los archivos `segment.c`, `fidtrackX.c`, `topologysearch.c` y `treeidmap.cpp`, e incluyendo su cabecera pública.
...
- Añadido un nuevo target `appimage` en el `Earthfile` que reutiliza la imagen base `the-base`, compila los binarios en modo Release y prepara un `AppDir` con `RectaiTable` y `soundtable-tracker` en `usr/bin`.
- El target descarga `linuxdeploy` y `appimagetool` como AppImages y los usa para recolectar todas las dependencias compartidas (incluyendo `curl`, OpenCV y librerías del sistema necesarias para JUCE) y empaquetarlas en un único `soundtable-x86_64.AppImage`.
...
- El AppImage resultante se exporta como artefacto local en `build/soundtable-x86_64.AppImage` al ejecutar `earthly -P +appimage` desde la raíz del repositorio.
...
### Vista de depuración de cámara en `soundtable-tracker`
- Añadido flag `--debug-view` al ejecutable `soundtable-tracker`.
- Cuando se pasa este flag (en cualquier modo), el proceso abre una ventana de OpenCV titulada `soundtable-tracker debug` donde muestra el frame de la webcam espejado horizontalmente (`cv::flip`), permitiendo verificar que la cámara está capturando correctamente y que el área de trabajo está bien encuadrada.
- Mientras la vista de depuración está activa, pulsar `Esc`, `q` o `Q` cierra la ventana y termina el tracker con un mensaje de log `[soundtable-tracker] Debug view exit requested by user`.

### Estabilización de fiduciales por frames consecutivos en `soundtable-tracker`
- En el modo `--mode=live`, el bucle principal de `soundtable-tracker` mantiene ahora un contador por ID de fiducial con el número de frames consecutivos en los que se ha detectado cada marcador.
- Solo se consideran "objetos estables" aquellos fiduciales que se han visto al menos en un umbral configurable de **N frames consecutivos** (actualmente 10); únicamente estos objetos se usan para actualizar `TrackerState` y para enviar mensajes OSC `/soundtable/object` al core JUCE.
...
- Las eliminaciones (`/soundtable/remove`) siguen basándose en `TrackerState::collectRemovals`, pero este estado solo se alimenta con los objetos estables, por lo que también se reducen los mensajes de "fiducial X removed" causados por falsos positivos de la cámara.
