...
- Ajustado el log `[soundtable-core][filter-routing]` en `MainComponent_AudioRouting` para que solo se emita cuando cambian los parámetros efectivos del filtro global de Sampleplay/Loop; se guarda el último estado publicado y se compara con tolerancias pequeñas antes de escribir en consola, eliminando el spam continuo cuando los módulos están docked o no hay cambios en la escena.
...
Refresco constante a 60 FPS
---------------------------

- Eliminado el filtro `hasVisualActivity` que bloqueaba los repaints cuando la escena parecía inactiva; ahora el `timerCallback` siempre programa repaints limitados a 60 Hz.
- El método `repaintWithRateLimit` sigue imponiendo el tope de 60 FPS, de modo que los pulsos centrales y las ondas permanecen fluidas sin disparar más trabajo del necesario en el pipeline de JUCE.

Reverb ligada a conexiones del Delay
------------------------------------

- El bus global de Delay/Reverb ahora trata ambos modos como envíos puros: la señal seca se mezcla siempre y la porción húmeda solo se suma en las voces, loops o Sampleplay que están conectados al Delay.
- La rama Sampleplay y el mezclador de Loops mantienen sus rutas secas incluso en modo reverb, acumulando por separado la señal que alimenta al efecto para evitar que el Reverb se active en módulos no conectados.
- El modo global añade únicamente la cola húmeda resultante del Delay/Reverb sobre la mezcla existente, asegurando que el reverb se comporte con los mismos ajustes que el delay tradicional.

Delay feedback vs reverb simultáneos
------------------------------------

- Separada la selección de destinos del Delay en `MainComponent_AudioRouting` para tratar los modos feedback y reverb de forma independiente, con estructuras `DelaySelection` dedicadas.
- Cada modo construye su propio `DelayTargetConfig` y se sincroniza con el motor de audio mediante llamadas separadas a `setDelayFxParams`, permitiendo que coexistan dos módulos Delay (uno en feedback y otro en reverb) sin interferencias.
- Cuando un modo no tiene fuentes conectadas se desactiva explícitamente antes de llegar al motor, evitando que la reverb o el feedback se apliquen fuera de sus conexiones.
- El `AudioEngine` mantiene ahora dos buses FX independientes (delay y reverb) con colas y ruteos separados, por lo que ambos modos pueden sonar al mismo tiempo según las conexiones activas.
- Al desconectar una fuente, el bus correspondiente permanece activo sin entradas nuevas mientras la cola se drena, así el reverb acumulado sigue sonando aunque el módulo pierda su conexión.
