...
### Sampleplay + FluidSynth: notas reales desde SoundFont
- Añadida una pequeña envoltura `SampleplaySynth` en `core/src/core/SampleplaySynth.{h,cpp}` que encapsula FluidSynth (`fluid_settings_t` + `fluid_synth_t`) y expone una API mínima para `setSampleRate`, `loadSoundfont`, `noteOn`/`noteOff` y `render` estéreo en buffers `float*`. Esta clase vive en `rectai` sin depender de JUCE y se protege con un `std::mutex` interno para hacer las llamadas thread-safe, asumiendo que la carga de trabajo actual es baja y aceptable incluso desde el hilo de audio.
- `AudioEngine` (`core/src/AudioEngine.{h,cpp}`) ahora mantiene un `std::unique_ptr<rectai::SampleplaySynth>` y dos buffers de trabajo (`sampleplayLeft_`, `sampleplayRight_`). En `audioDeviceAboutToStart` configura el sample rate de FluidSynth y dimensiona los buffers según el tamaño máximo de bloque del dispositivo. En el callback de audio:
