...
### Separación de waveform cruda y filtrada de Sampleplay y alineación de cortes de radiales
- En `AudioEngine` (`AudioEngine.h/.cpp`) se ha dividido el historial de waveform de Sampleplay en dos buffers: `sampleplayWaveformBuffer_` pasa a almacenar la señal original del SoundFont después de aplicar el gain global pero **antes** del filtro dedicado de Sampleplay, y se introduce `sampleplayFilteredWaveformBuffer_` para guardar la señal tras dicho filtro. Se añade además el método `getSampleplayFilteredWaveformSnapshot(...)` que devuelve snapshots de la señal filtrada análogo a `getSampleplayWaveformSnapshot` (que ahora se documenta y utiliza como waveform cruda).
- El callback de audio actualiza ambos historiales en cada muestra: calcula `sampleplayMonoRaw` a partir de los canales izquierdo/derecho gananciados, pasa esa señal por el filtro TPT de Sampleplay para obtener `sampleplayMonoFiltered` y escribe cada valor en su buffer correspondiente. Los taps por conexión de tipo `kSampleplay` se alimentan siempre de la señal cruda para mantener una única referencia de waveform base.
- En `MainComponent_Paint.cpp`, el pintado de Sampleplay se apoya ahora en dos snapshots distintos: las radiales `Sampleplay → Master` y las conexiones que involucran Sampleplay usan siempre `sampleplayWaveformRaw` (onda original), mientras que las radiales de `Filter` con Sampleplay aguas arriba utilizan `sampleplayWaveformFiltered` para mostrar la señal ya filtrada. Con ello, en una cadena `Sampleplay → Filter → Master` la waveform de `Sampleplay → Filter` coincide exactamente con la que se vería en `Sampleplay → Master`, y la radial `Filter → Master` refleja el resultado del filtro.
- La lógica de pintado de conexiones que involucran Sampleplay deja de depender de taps por conexión y pasa a reutilizar el buffer global `sampleplayWaveformRaw` con el mismo mapeo que la radial del módulo, desactivando el modo "tiled" para que se represente una única ventana continua de audio. Además, las conexiones Sampleplay solo se consideran muteadas cuando su clave aparece explícitamente en `mutedConnections_`; los mutes propagados desde la radial al master (`sourceMutedToMaster`) ya no silencian visualmente las conexiones Sampleplay→módulo, de forma que seguirán mostrando waveform mientras haya sonido en el path global.
- En `MainComponent_Input.cpp` se ha alineado la heurística de qué radiales pueden cortarse con la usada en pintado, tratando como "generator-like" a cualquier módulo cuyo tipo **no** sea `ModuleType::kSettings`. Durante un gesto de corte en el área musical, si un módulo generator-like tiene al menos una conexión saliente activa hacia otro módulo, su radial centro→módulo se considera oculta y se omite del hit-test de corte. Esto hace que, cuando Sampleplay alimenta a un Filter, su radial directa a Master (que ya no se dibuja) tampoco pueda cortarse, evitando que un gesto en espacio vacío mute una línea invisible y deje sin sonido la cadena `Sampleplay → Filter`.
