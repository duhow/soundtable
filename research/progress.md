...
- Se ha añadido un helper `waveform_index()` en `rectai::OscillatorModule` (`core/src/core/AudioModules.h`) que expone el valor entero asociado al enum `Waveform` (`kSine`, `kSaw`, `kSquare`, `kNoise`) siguiendo el orden ya usado internamente por el módulo. De este modo, cualquier código que necesite un índice de forma de onda compatible con el motor de audio (`AudioEngine::setVoiceWaveform`) puede obtenerlo directamente del módulo sin re-codificar el mapeo.
- En `MainComponent_Audio.cpp`, dentro de `timerCallback`, se ha simplificado la selección de la forma de onda para cada voz de generador: en lugar de volver a mapear manualmente `OscillatorModule::Waveform` a índices 0–3 mediante una cadena de `if`/`else`, ahora se hace un `dynamic_cast` a `OscillatorModule` y se llama a `waveform_index()`. Esto reduce duplicación de lógica y garantiza que cualquier futura ampliación del enum o cambios en el orden de las formas de onda sólo requieran tocar la implementación del módulo, manteniendo a la UI y al motor sincronizados.
